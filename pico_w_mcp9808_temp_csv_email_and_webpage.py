...
import utime
import _thread
import MCP9808 # this needs to be a file saved on device
import umail   # this needs to be a file saved on device
import os
...
def log_temp_data_to_file(FILE_NAME='temp_data.csv',meas_interval=30, fahrenheit=True): # meas_interval is in minutes
    i2c = machine.I2C(0,sda=machine.Pin(0), scl=machine.Pin(1), freq=400_000) # pico w
...
def web_server(FILE_NAME = 'temp_data.csv'):
    while True:
        ...
        address = socket.getaddrinfo('192.168.0.202', 80)[0][-1]
...
def sendEmail(CSV_FILE):
    ...
    smtp.write("PICO W Temperature logfile is attached\n")
...
while (not connect_to_wifi()):
    time.sleep(10)
# Pico W has 2 cores, let's have temperature logging run on one thread
core1_thread = _thread.start_new_thread(log_temp_data_to_file, ())
